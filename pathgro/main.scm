...
(define (main args) 
  (let* 
    ((option-spec 
     ...
     (opt-traverse     (option-ref options 'traverse #f))
    ...
    (when opt-traverse
      (let ((cps (path-combos opt-Combos (list) dirns)))
        (output-list (path-traverse (prepend-slashes (flatten cps))))))
	  (when (< 0 opt-Combos) 
