...
(use-modules ((pathgro base pathgro-compute) #:select (pathgro-compute combine-paths)))
...
(define (main args)
  (let* ((option-spec '((help    (single-char #\h))
                  ...
                  (opt-extname    (option-ref options 'extname #f))
                  (opt-level      (string->number (option-ref options 'level "4096")))
    ...
    (cond
      ...
      (else
        (let ((stripped-args (option-ref options '() '()))
          ...
          (when pathgro-debug (begin (display "stripped-args: ")
                                     (display stripped-args)
                                     (newline)))
          (read-pathsfiles stripped-args)
          (when pathgro-debug (begin (display "opt-level: ")
                                     (display opt-level)
                                     (newline)))
          (when pathgro-debug (begin (display "bases: ")
                                     (display bases)
                                     (newline)))
          (when pathgro-debug (begin (display "extns: ")
                                     (display extns)
                                     (newline)))
          (let ((cfiles (uniq (unblank (combine-paths opt-level bases extns)))))
            (when pathgro-debug (begin (display "cfiles: ")
                                       (display cfiles)
                                       (newline)))
            (letrec ((edirs (combine-paths opt-level dirns extns))
                  (cdirs (append dirns edirs)))
              (when pathgro-debug (begin (display "edirs: ")
                                         (display edirs)
                                         (newline)))
              (when pathgro-debug (begin (display "cdirs: ")
                                         (display cdirs)
                                         (newline)))
                (if (or opt-basename opt-dirname opt-extname opt-filename opt-extname opt-powerset opt-extdirname)
                  (begin
                    ...
                    (when opt-powerset (for-each println (pathgro-compute opt-level cfiles))))
                  (display-help))
                (exit 0))))))))
