    ...
    (cond
      ...
      (else
        (let ((stripped-args (option-ref options '() '())))
          ...
          (let ((folder-depth 4096)
                ;(edirs (flatten (uniq (unblank (combine-paths dirns extns)))))
                (edirs (flatten (combine-paths dirns extns)))
            ...
            (let-values ((cfiles cdirs) (values (uniq (unblank (combine-paths bases extns))) (uniq (unblank (append dirns edirs)))))
                ...
                (if (not (or opt-basename opt-dirname opt-extname opt-filename opt-extname opt-powerset opt-extdirname))
                  (display-help)
                  (begin
                    (when opt-level  (set! folder-depth opt-level))
                    ...
                    ;(when opt-powerset (for-each println (pathgro-full folder-depth cfiles)))))
                    (when opt-powerset (for-each println (pathgro-full folder-depth efiles)))))
                (exit 0))))))))
