...
(define (display-help)
  (newline)
  (display (colorize-string "usage" 'WHITE 'ON-YELLOW 'BOLD 'UNDERLINE))
  (display (colorize-string ": " 'BOLD))
  (display (colorize-string "pathgro" 'WHITE 'ON-GREEN 'BOLD))
  (display " ")
  (display (colorize-string "[<OPTIONS>]" 'WHITE 'ON-BLUE 'UNDERLINE))
  (display " ")
  (display (colorize-string "[<FILES>]" 'WHITE 'ON-BLUE 'UNDERLINE))
  (newlines 2)
  (display "  ")
  (display (colorize-string "<OPTIONS>" 'WHITE 'ON-BLUE 'UNDERLINE))
  (display "            ")
  ...
  (display "       ")
  (display (colorize-string "print full filenames, i.e. \"basename.extname\"" 'GREEN))
  (newline)
  (display "  ")
  ...
  (newline)
  (display "  ")
  (display (colorize-string "-c, --compute" 'BLACK 'ON-GREEN 'BOLD 'UNDERLINE))
  (display "        ")
  (display (colorize-string "compute all permutations and combinations of pathnames" 'GREEN))
  (newline)
  (display "  ")
  ...
  (display "  ")
  (display (colorize-string "DEPTH" 'BLACK 'ON-BLUE 'REVERSE 'UNDERLINE))
  (display "   ")
...
(define (display-version)
  ...
  (display (colorize-string "]" 'YELLOW 'BOLD))
  (newlines 2)
  (display (colorize-string "This program comes with ABSOLUTELY NO WARRANTY. It is free software and you are welcome to redistribute it under certain conditions. See the COPYING.txt file in the root directory of the source repository for details." 'RED))
  (newlines 2)
  ...
  (let* 
    ...
    (cond
      ...
      (version-wanted
        (display-version))
      (else
        ;(when (or opt-basename opt-dirname opt-extname opt-filename opt-extdirname)
        ;      (when opt-compute (set! opt-compute #f)))
        (let ((stripped-args (option-ref options '() '()))
          ...
          (let ((cfiles (clean (combine-files-helper bases extns)))
                ...
                (when (not (zero? opt-level)) (set! max-depth opt-level)) 
                (let ((aps (path-powerset cfiles dirns)))
                  (when opt-compute (output-list aps))))))))
            (exit 0))
